# The goal of the project is to clean and extract usable data from the given zip file.
# R script, run_analysis.R, merges the training and test sets to create one data set,
# Extracts only the measurements on the mean and standard deviation for each measurement
# uses descriptive activity names to name the activities in the data set, appropriately
# labels the data set with descriptive variable names. 
# From the data set in step 4, creates an independent tidy data set with the average of
...
# The zip downloaded from # #https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip is #and extracted into home directory ( working directory)
...
#The supporting metadata in this data are loaded into variables featureNames and activityLabels.

featureNames <- read.table("UCI HAR Dataset/features.txt")
activityLabels <- read.table("UCI HAR Dataset/activity_labels.txt", header = FALSE)

#Format training and test data sets
#Both training and test data sets are availavle in subject, activity and features,
...
# We can use combine the respective data in training and test data sets corresponding
# to subject, activity and features. The results are stored insubject, activity and
# features.
subject <- rbind(subjectTrain, subjectTest)
activity <- rbind(activityTrain, activityTest)
features <- rbind(featuresTrain, featuresTest)

# The columns in the features data can be named from the metadata in featureNames

colnames(features) <- t(featureNames[2])

#Merge the data: The data in features,activity and subject are merged. The complete data
#is now stored in completeData.
...
# each measurement. Extract the column indices that have either mean or std in them
...
# change its type to character so that it can accept activity names, that are taken 
# from metadata activityLabels.
...
# Then, we order the enties in tidyData and write it into data fileTidy.txt that 
